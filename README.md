...
### Model
`Model` is abstracted to construct the forward network which defines a policy network or critic network given state as input. PARL also provides a helpful API `sync_params_to` in `Model`, which can be used to synchronize parameters between model and target model directly.
