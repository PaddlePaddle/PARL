...
class MAML(parl.Algorithm):
    def __init__(self, model: MAMLModel, config: Config, device: torch.device):
        ...
        self.learning_rates = nn.Parameter(
                data=torch.ones((config.num_updates_per_iter, config.num_layers), device=device) * config.task_learning_rate,
                requires_grad=config.learnable_learning_rates)

        self.optimizer = optim.Adam(self.model.get_weights() + [self.learning_rates], lr=config.meta_learning_rate, amsgrad=False)
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer=self.optimizer, T_max=config.total_epochs,
    ...
    def train_one_iter(self, data_batch: Tuple, current_epoch: int) -> float:
        ...
        if self.config.learning_rate_scheduler: self.scheduler.step(current_epoch)
        batch_losses = []
        for x_support, y_support, x_query, y_query in zip(x_support_set, y_support_set, x_query_set, y_query_set):
            task_loss = 0
            per_step_loss_importance_vectors = self._get_per_step_loss_importance_vector(current_epoch)
            model_weights = self.model.get_weights()

            for num_step in range(self.config.num_updates_per_iter):

                support_loss = self._caculate_loss(x_support, y_support, model_weights)

                model_weights = self._inner_loop_update(loss=support_loss,
                ...
                if self.config.use_multi_step_loss_optimization and current_epoch < self.config.multi_step_loss_num_epochs:
                    query_loss = self._caculate_loss(x=x_query, y=y_query, weights=model_weights)
                    task_loss = task_loss + per_step_loss_importance_vectors[num_step] * query_loss
    ...
    def evaluate_one_iter(self, data_batch: Tuple) -> float:
        ...
        for x_support, y_support, x_query, y_query in zip(x_support_set, y_support_set, x_query_set, y_query_set):
            ...
            with torch.no_grad():
                query_loss = self._caculate_loss(x_query, y_query, model_weights)
    ...
    def _inner_loop_update(self, loss: torch.Tensor, weights: List, use_second_order: bool,
        ...
        grads = torch.autograd.grad(loss, weights, create_graph=use_second_order, allow_unused=True)

        updated_weights = list()

        for idx, (weight, grad) in enumerate(zip(weights, grads)):
            updated_weight = weight - self.learning_rates[current_update_step, idx // 2] * grad
    ...
    def _outer_loop_update(self, loss):
        ...
        loss.backward()
        if self.learning_rates.requires_grad: self.learning_rates.grad.clamp_(-20, 20)
        self.optimizer.step()
    ...
    def _get_per_step_loss_importance_vector(self, epoch: int) -> torch.Tensor:
        ...
        """
        loss_weights = np.ones(self.config.num_updates_per_iter) / self.config.num_updates_per_iter
    ...
    def _caculate_loss(self, x: torch.Tensor, y: torch.Tensor, weights: List) -> torch.Tensor:
