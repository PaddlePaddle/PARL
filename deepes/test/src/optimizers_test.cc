...
// Tests that the Optimizers::compute_centered_rank() method.
TEST(SGDOptimizersTest, Method_update) {
	std::shared_ptr<Optimizer> optimizer = std::make_shared<SGDOptimizer>(1.0);
...
TEST(AdamOptimizersTest, Method_update) {
	std::shared_ptr<Optimizer> optimizer = std::make_shared<AdamOptimizer>(1.0);
