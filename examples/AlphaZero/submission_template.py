...
class SimpleAgent():
    ...
    def load_checkpoint(self, str_weights, weights_shape):
        dict_weights = eval(str_weights)
        paddle_weight = {}
        for k, v in dict_weights.items():
            convert_array = np.frombuffer(dict_weights[k], dtype='float32')
            convert_array = np.reshape(convert_array, weights_shape[k])
            paddle_weight[k] = convert_array
        self.model.set_state_dict(paddle_weight)
...
agent.load_checkpoint(str_weights, weights_shape)
