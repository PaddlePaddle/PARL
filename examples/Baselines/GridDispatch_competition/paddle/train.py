...
class Learner(object):
    def __init__(self, args):
        ...
        parl.connect(args.xparl_addr, distributed_files=[
            'lib64/*',
            'model_jm/*',
            'Agent/*',
            'Environment/*',
    ...
    def run_sampling(self):
        ...
        while True:
            ...
            sample_data, episode_steps, episode_reward = actor.sample(weights, random_action)
            ...
            mean_action = np.mean(np.array([x[1] for x in sample_data]), axis=0)

            with self.log_lock:
                self.total_steps += episode_steps
                self.total_MDP_steps += len(sample_data)
                tensorboard.add_scalar('train/episode_reward', episode_reward, self.total_steps)
                tensorboard.add_scalar('train/episode_steps', episode_steps, self.total_steps)
                if critic_loss is not None:
                    tensorboard.add_scalar('train/critic_loss', critic_loss, self.total_steps)
                    tensorboard.add_scalar('train/actor_loss', actor_loss, self.total_steps)
                logger.info('Total Steps: {} Reward: {} Steps: {}'.format(self.total_steps, episode_reward, episode_steps))
                ...
                if self.total_steps // self.args.save_every_steps >= self.save_cnt:
                    while self.total_steps // self.args.save_every_steps >= self.save_cnt:
                        self.save_cnt += 1
                    with self.model_lock:
                        self.agent.save(os.path.join(self.args.save_dir, "model-{}".format(self.total_steps)))
