...
def get_world_feature(board):
    ...
    map_halite = np.array(board.observation['halite']).reshape(1, size, size) / 50

    for _, ship in board.ships.items():
        ships[0, size - ship.position[1] - 1, ship.position[0]] = 1 if ship.player_id == me.id else -1
        ship_cargo[0, size - ship.position[1] - 1, ship.position[0]] = ship.halite
    
    for _, yard in board.shipyards.items():
        bases[0, size - yard.position[1] - 1, size - yard.position[0] - 1] = 1 if yard.player_id == me.id else -1
...
def get_distance_map(position):
    ...
    distance_x = np.where(delta_distance_x < offset_distance_x, delta_distance_x, offset_distance_x)
    distance_y = np.where(delta_distance_y < offset_distance_y, delta_distance_y, offset_distance_y)
