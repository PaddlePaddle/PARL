...
class ReplayMemory(object):
    ...
    def sample_batch(self, batch_size):
        ...
        action = self.action[batch_idx].reshape((-1, 1))
        value = self.value[batch_idx].reshape((-1, 1))
        returns = self.returns[batch_idx].reshape((-1, 1))
        log_prob = self.log_prob[batch_idx].reshape((-1, 1))
        adv = self.adv[batch_idx].reshape((-1, 1))
