...
class MAAgent(parl.Agent):
    ...
    def build_program(self):
        ...
        with fluid.program_guard(self.learn_program):
            obs_n = [layers.data(name='obs'+str(i), shape=[self.obs_dim_n[i]], 
                                    dtype='float32') for i in range(self.n)]
            act_n = [layers.data(name='act'+str(i), shape=[self.act_dim_n[i]], 
                                    dtype='float32') for i in range(self.n)]
        ...
        if self.speedup:
            ...
            self.learn_program = parl.compile(self.learn_program, self.critic_cost)
    ...
    def learn(self, agents):
        ...
        for _ in range(num_sample):
            ...
            feed = {**feed_obs, **feed_act} # merge two dict
        ...
        feed = {**feed_obs, **feed_act, 'target_q': target_q}
