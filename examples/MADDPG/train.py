...
def run_episode(env, agents):
    ...
    while True:
        ...
        for i, agent in enumerate(agents):
            ...
            logger.add_scalar('critic_loss_%d' % i, critic_loss,
        ...
        if total_episodes % args.stat_rate == 0:
            ...
            logger.info(
                ...
                format(total_steps, total_episodes, mean_episode_reward,
                       use_time))
            t_start = time.time()
            logger.add_scalar('mean_episode_reward/episode',
                                   mean_episode_reward, total_episodes)
            logger.add_scalar('mean_episode_reward/steps',
                                   mean_episode_reward, total_steps)
            logger.add_scalar('use_time/1000episode', use_time,
                                   total_episodes)
