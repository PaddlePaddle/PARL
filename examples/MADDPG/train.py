...
def run_episode(env, agents):
    ...
    while True:
        ...
        for i, agent in enumerate(agents):
            agent.add_experience(obs_n[i], action_n[i], 
                        reward_n[i], next_obs_n[i], done_n[i]) 
        ...
        if args.restore and args.show: # show model effect without training
            continue
            
        # learn policy
        for i, agent in enumerate(agents):
            critic_loss = agent.learn(agents)
            tensorboard.add_scalar('critic_loss_%d'%i, critic_loss, agent.global_train_step)
...
def train_agent():
    ...
    for i in range(env.n): # attention: 'Discrete' object has no attribute 'low'
        ...
        if('low' in dir(env.action_space[i])):
            ...
            logger.info('num_discrete_space:{}'.format(env.action_space[i].num_discrete_space))
    ...
    while total_episodes <= args.max_episodes:
        ...
        if total_episodes % args.stat_rate == 0:
            ...
            logger.info('Steps: {}, Episodes: {}, Mean episode reward: {}, Time: {}'.format(
                            total_steps, total_episodes, mean_episode_reward, use_time))
            t_start = time.time()
            tensorboard.add_scalar('mean_episode_reward/episode', mean_episode_reward, total_episodes)
            tensorboard.add_scalar('mean_episode_reward/steps', mean_episode_reward, total_steps)
            tensorboard.add_scalar('use_time/1000episode', use_time, total_episodes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_step_per_episode', type=int, default=25, 
    ...
    parser.add_argument('--stat_rate', type=int, default=1000, 
