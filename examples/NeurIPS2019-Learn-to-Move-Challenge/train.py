...
from parl.utils import logger, ReplayMemory, get_gpu_count
...
class Learner(object):
    def __init__(self, args):
        ...
        # add lock when appending data to rpm or writing scalars to logger
        ...
        with self.memory_lock:
            ...
            if actor_state.ident % 3 == 2:  # trajectory without noise
                ...
                if self.env_reward_stat.count > 500:
                    logger.add_scalar('recent_env_reward',
                                           self.env_reward_stat.mean,
                                           self.total_steps)
                    logger.add_scalar('recent_shaping_reward',
                                           self.shaping_reward_stat.mean,
                                           self.total_steps)
                if self.critic_loss_stat.count > 500:
                    logger.add_scalar('recent_critic_loss',
                                           self.critic_loss_stat.mean,
                                           self.total_steps)
                logger.add_scalar('episode_length', n, self.total_steps)
                logger.add_scalar('max_env_reward', self.max_env_reward,
                                       self.total_steps)
                logger.add_scalar('ready_actor_num',
                                       self.ready_actor_queue.qsize(),
                                       self.total_steps)
                logger.add_scalar('episode_time', episode_time,
                                       self.total_steps)
