...
class PolicyModel(Model):
    def __init__(self, obs_dim, act_dim, init_logvar):
        ...
        self.fc1 = layers.fc(size=hid1_size, act='tanh', 
                             param_attr=ParamAttr(initializer = fluid.initializer.Normal(scale=np.sqrt(1 / obs_dim))))
        self.fc2 = layers.fc(size=hid2_size, act='tanh',
                             param_attr=ParamAttr(initializer = fluid.initializer.Normal(scale=np.sqrt(1 / hid1_size))))
        self.fc3 = layers.fc(size=hid3_size, act='tanh',
                             param_attr=ParamAttr(initializer = fluid.initializer.Normal(scale=np.sqrt(1 / hid2_size))))
        self.fc4 = layers.fc(size=act_dim, act='tanh',
                             param_attr=ParamAttr(initializer = fluid.initializer.Normal(scale=np.sqrt(1 / hid3_size))))

        self.logvars = layers.create_parameter(shape=[act_dim], dtype='float32',
