...
class CQL(parl.Algorithm):
    def __init__(self,
        ...
        if self.with_automatic_entropy_tuning:
            ...
            self.alpha_optimizer = paddle.optimizer.Adam(learning_rate=actor_lr, parameters=[self.log_alpha])

        if self.with_lagrange:
            self.target_action_gap = self.lagrange_thresh
            self.log_alpha_prime = paddle.zeros([1])
            self.alpha_prime_optimizer = paddle.optimizer.Adam(learning_rate=critic_lr, parameters=[self.log_alpha_prime])
    ...
    def sample(self, obs):
        ...
        x_t = normal.sample([1])#act_mean + self.sample_normal.sample(act_log_std.shape) * act_log_std.exp()
        x_t= x_t.squeeze()
    ...
    def _get_policy_actions(self, obs):
        ...
        return new_obs_actions, new_obs_log_pi.reshape([obs.shape[0],
                                                    self.num_random, 1])
