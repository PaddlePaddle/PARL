...
class DiagGaussianDistributionTest(unittest.TestCase):
    def setUp(self):
        ...
        self.std = paddle.rand(shape=(self.batch_size, self.num_actions))
    ...
    def get_dist(self):
        ...
        std = paddle.rand(shape=(self.batch_size, self.num_actions))
        logits = (mean, std)
...
class SoftCategoricalDistributionTest(unittest.TestCase):
    ...
    def test_entropy(self):
        ...
        #     np.log(self.num_actions) + ep)
        # # range check of entropy, the minimum entropy should be higher than zero
        # self.assertTrue(paddle.min(entropy) >= 0)
