...
def start_master(port, cpu_num, monitor_port, debug):
    ...
    if _IS_WINDOWS:
        # os.devnull will cause bugs in Windows with flask
        monitor_log = open('monitor.log', 'w')
        _ = subprocess.Popen(monitor_command, stdout=monitor_log)
        monitor_log.close()
    else:
        _ = subprocess.Popen(monitor_command, stdout=FNULL, stderr=subprocess.STDOUT)
...
def stop():
    """Stop process related to parl.
    
    Windows only has: taskkill /F /IM "python.exe" which is too brutal.
    """
    if not _IS_WINDOWS:
