function createDivs(res, divs) {
  ...
  if (curr_num < divs) {
    for (var i = curr_num; i < divs; i++) {
      ...
      workerDiv.id = `w${i}`;
      if (i === 0) {
        workerDiv.innerHTML = `<div class="card-header" id="${i}"><div style="display:inline;">Master</div><div style="float:right; display:inlene;" id="gpu">[GPU] ${res.total_vacant_gpus}/${res.total_gpus}</div></div>`;
      } else {
  ...
  for (var i = begin; i < end; i++) {
      ...
      series: [
        {
          ...
          data: [
            { value: worker.used_gpus, name: "Used GPU" },
            { value: worker.vacant_gpus, name: "Vacant GPU" }
          ]
        }
      ]
    };
    ...
    var memoryOption = {
      ...
      legend: {
        orient: "vertical",
        x: "left",
        data: ["Used GPU Memory", "Vacant GPU Memory"],
        textStyle: {
          fontSize: 8,
        }        
      },
      series: [
        {
          ...
          data: [
            { value: worker.used_gpu_memory, name: "Used GPU Memory" },
            { value: worker.vacant_gpu_memory, name: "Vacant GPU Memory" }
          ]
        }
      ]
    };
    ...
    var gpuNum = document.getElementById('gpu');
    gpu.innerText = `[GPU] ${res.total_vacant_gpus}/${res.total_gpus}`

    if (i < record_num && worker.hostname === record[i].hostname) {
      if (worker.used_gpus !== record[i].used_gpus) {
        imgHandle[`w${i}c0`].setOption(gpuOption);
      }
      if (worker.used_gpu_memory !== record[i].used_gpu_memory) {
    ...
    record[i] = {
      ...
      used_gpus: worker.used_gpus,
      vacant_gpus: worker.vacant_gpus,
      used_gpu_memory: worker.used_gpu_memory,
      vacant_gpu_memory: worker.vacant_gpu_memory
    };
  }
