...
class TestCluster(unittest.TestCase):
    ...
    def test_actor_exception(self):
        ...
        master = Master(port=8335)
        th = threading.Thread(target=master.run)
        th.start()
        time.sleep(3)
        worker1 = Worker('localhost:8335', 1)
        for _ in range(3):
            if master.cpu_num == 1:
                break
            time.sleep(10)
    ...
    def test_actor_exception_2(self):
        ...
        master = Master(port=8336)
        th = threading.Thread(target=master.run)
        th.start()
        time.sleep(3)
        worker1 = Worker('localhost:8336', 1)
        self.assertEqual(1, master.cpu_num)
        parl.connect('localhost:8336')
        actor = Actor()
        try:
    ...
    def test_reset_actor(self):
        ...
        # start the master
        master = Master(port=8337)
        th = threading.Thread(target=master.run)
        th.start()
        time.sleep(3)

        worker1 = Worker('localhost:8337', 4)
        parl.connect('localhost:8337')
        for _ in range(10):
            actor = Actor()
            ret = actor.add_one(1)
    ...
    def test_add_worker(self):
        ...
        master = Master(port=8334)
        th = threading.Thread(target=master.run)
        th.start()
        time.sleep(1)

        worker1 = Worker('localhost:8334', 4)
        for _ in range(3):
            if master.cpu_num == 4:
                break
            time.sleep(10)
        ...
        worker2 = Worker('localhost:8334', 4)
        for _ in range(3):
            if master.cpu_num == 8:
                break
            time.sleep(10)
